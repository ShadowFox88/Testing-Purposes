...
local function onInputBegan(input, inGui)
	if input.KeyCode ~= Enum.KeyCode.J or input.UserInputState ~= Enum.UserInputState.Begin or inGui then
		return
	end
	...
	for i = 1, 360 do
		...
		part.Anchored = true
		part.CFrame = origin * offset * CFrame.Angles(0, math.rad(i), 0) * CFrame.new(0, 0, -10) * getRandomAngles()
		part.Parent = workspace
	end
end

UserInputService.InputBegan:Connect(Utils.debounce(onInputBegan))
